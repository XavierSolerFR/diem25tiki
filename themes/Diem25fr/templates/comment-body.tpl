...
<div class="postbody-content panel-body row" >
	<div class="col-md-10">
	...
	<div class="col-md-2 author" style="border-left: gray;
		...
		<div class="author_info">
			...
			<div>
				...
				{if isset($comment.anonymous_name) and $comment.anonymous_name}
					<span class="author_post_info_by">{if $comment.website}<a href="{$comment.website}" target="_blank">{/if}{$comment.anonymous_name}{if $comment.website}</a>{/if}</span>
				{elseif isset($comment.userName)}
					<span class="author_post_info_by">{$comment.userName|userlink}</span>
				{/if}
		...
		{if isset($first) and $first eq 'y'}
			...
			{tr}Reads{/tr}: {$comment.hits}
		</span>
		{else}
			<div class="back_to_top pull-right" >
