...
{capture name="parentsforum"}
	<li><a class="link" href="{if $prefs.feature_sefurl eq 'y'}forums{else}tiki-forums.php{/if}">{tr}Forums{/tr}</a></li>
	<li><a class="link" href="{$forumId|sefurl:'forum'}">{$forum_info.name|addongroupname|escape}</a></li>
	{if isset($thread_info.topic.threadId) and $thread_info.topic.threadId}
	<li><a class="link" href="{$thread_info.topic.threadId|sefurl:'forumthread'}{if $smarty.request.topics_offset}&amp;topics_offset={$smarty.request.topics_offset}{/if}{$topics_sort_mode_param}{$topics_threshold_param}{$topics_find_param}">
			{$thread_info.topic.threadId}
		</a></li>
	{/if}
{/capture}
{capture name="threadActions"}
	...
	<li>
		<a href="tiki-admin_forums.php?forumId={$forumId}">{tr}Edit Forum{/tr}</a>
	</li>
	{/if}
{/capture}
...
<div class="row">
	{block name="bread_title"}
			...
			{bread_title help="forum" parents=$smarty.capture.parentsforum
			admpage="forums" actionsAdd=$smarty.capture.threadActions }{$thread_info.title}{/bread_title}
