...
{block name="bread_title"}
		...
		{capture name="parentArticle"}
			...
			<li><a class="link" href="{if $prefs.feature_sefurl eq 'y'}articles{else}tiki-view_articles.php{/if}?topic={$topicId}">{$topicName}</a></li>
		{/capture}
		{bread_title help='' parents=$smarty.capture.parentArticle	admpage="" actionsAdd="" }{$arttitle}{/bread_title}
