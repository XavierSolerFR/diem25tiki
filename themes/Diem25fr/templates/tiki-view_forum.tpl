...
{if !$tsAjax}
	...
	{block name=bread_title}
		...
		{bread_title help="forum" parents='<li><a href="forums">{tr}Forums{/tr}</a></li>'
             admpage="forums"  url=$forum_info.forumId|sefurl:'forum'}{$forum_info.name|addongroupname}{/bread_title}
        {else}
            {bread_title help="forum" parents='<li><a href="tiki-forums.php">{tr}Forums{/tr}</a></li>'
            admpage="forums"  url=$forum_info.forumId|sefurl:'forum'}{$forum_info.name|addongroupname}{/bread_title}
        {/if}
	{/block}
	...
	<div class="t_navbar row margin-bottom-md">
        <div class="col-md-7">
		...
		{if $tiki_p_admin_forum eq 'y' or !isset($all_forums) or $all_forums|@count > 1}
            ...
            {jq}
        {literal}$("#TitleBreadActions").append{/literal}('<li><a href="tiki-admin_forums.php?forumId={$thisforum_info}&amp;cookietab=2#content_admin_forums1-2"><span class="icon icon-edit fa fa-edit fa-fw "></span>{tr}Edit Forum{/tr}</a></li>');
            {/jq}
		{/if}
        {jq}
            vTitleBreadActions ="";
        {if $user and $prefs.feature_user_watches eq 'n'}
                vTitleBreadActions =vTitleBreadActions+'<li><a href="tiki-view_forum.php?forumId={$forumId}&amp;watch_event=forum_post_topic&amp;watch_object={$forumId}&amp;watch_action=add">'
                +'{icon name="watch"} {tr}Monitor topics{/tr}</a></li>';
       {else}
                vTitleBreadActions =vTitleBreadActions+'<li><a href="tiki-view_forum.php?forumId={$forumId}}&amp;watch_event=forum_post_topic&amp;watch_object={$forumId}&amp;watch_action=remove">'
                +'{icon name="stop-watching"} {tr}Stop monitoring topics{/tr}</a></li>';
        {/if}
        {if $user and $prefs.feature_user_watches eq 'y'}
            vTitleBreadActions =vTitleBreadActions+'<li><a href="tiki-view_forum.php?forumId={$forumId}&amp;watch_event=forum_post_topic_and_thread&amp;watch_object={$forumId}&amp;'
            {if $user_watching_forum_topic_and_thread eq 'n'}
                 +'watch_action=add">{icon name="watch"} {tr}Monitor topics and threads{/tr}'
             {else}
                +'watch_action=remove">{icon name="stop-watching"} {tr}Stop monitoring topics and threads{/tr}'
            {/if}
            +'</a></li>';
        {/if}
        {if $prefs.feature_group_watches eq 'y' and ( $tiki_p_admin_users eq 'y' or $tiki_p_admin eq 'y' )}
            vTitleBreadActions =vTitleBreadActions+'<li><a href="tiki-object_watches.php?objectId={$forumId|escape:"url"}&amp;watch_event=forum_post_topic&amp;objectType=forum&amp;objectName={$forum_info.name|escape:"url"}&amp;objectHref={'tiki-view_forum.php?forumId='|cat:$forumId|escape:"url"}">'
                  +'{icon name="watch-group"} {tr}Group monitor topics{/tr}</a></li>'
            +'<li><a href="tiki-object_watches.php?objectId={$forumId|escape:"url"}&amp;watch_event=forum_post_topic_and_thread&amp;objectType=forum&amp;objectName={$forum_info.name|escape:"url"}&amp;objectHref={'tiki-view_forum.php?forumId='|cat:$forumId|escape:"url"}">'
                  +'{icon name="watch-group"} {tr}Group monitor topics and threads{/tr}</a></li>;
        {/if}
        {if ('y'=='y') or (!empty($tiki_p_forum_lock) and $tiki_p_forum_lock eq 'y')}

            vTitleBreadActions =vTitleBreadActions+'<li class="divider"></li><li>'
        ...
        {if $prefs.feed_forum eq 'y'}
            vTitleBreadActions =vTitleBreadActions+'<li class="divider"></li>'
            +'<li><a href="tiki-forum_rss.php?forumId={$forumId}">{icon name="rss"} {tr}RSS feed{/tr}</a></li>';
        {/if}
            if (vTitleBreadActions != ""){
        {literal}$('#TitleBreadActions').append{/literal}('<li class="divider"></li>'+vTitleBreadActions);}
			...
			<form method="post" enctype="multipart/form-data" action="tiki-view_forum.php" id="editpageform">
				...
				<div class="form-horizontal">
					...
					<div class="row col-sm-12 form-group">
						...
						<div class="col-sm-10">
							{textarea id="editpost" class="form-control" name="comments_data"  codemirror="y" syntax="tiki" _toolbars=$prefs.feature_forum_parse}{$comment_data}{/textarea}
						</div>
					</div>
