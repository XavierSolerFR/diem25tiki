...
<header class="panel-heading clearfix postbody-title media-overflow-visible"> {*the panel-heading class will cut off dropdowns so need media-overflow-visible class *}
	{if $prefs.feature_comments_locking neq 'y' or	( $comment.locked neq 'y' and $thread_is_locked neq 'y' )}
		...
		<div class="actions pull-right btn-group">
			{if $thread_style != 'commentStyle_headers' and isset($comment.approved) and $comment.approved eq 'y'}
					...
					<a title="{tr}Reply{/tr}"
							{if $first eq 'y'}
					   			href="#form"
					  			 _onclick="show('{$postclass}open');"
							{elseif $comments_grandParentId}
								href="?post_reply=1&comments_threshold={$comments_threshold}&comments_reply_threadId={$comment.threadId}&comments_offset={$comments_offset}&thread_sort_mode={$thread_sort_mode}&comments_per_page={$comments_per_page}&thread_style={$thread_style}&comments_grandParentId={$comment_grandParentId}&comments_parentId={$comments_grandParentId}#form"
							{elseif $forum_info.is_flat neq 'y' or $prefs.feature_forum_allow_flat_forum_quotes eq 'y'}
								href="?post_reply=1&comments_threshold={$comments_threshold}&comments_reply_threadId={$comment.threadId}&comments_offset={$comments_offset}&thread_sort_mode={$thread_sort_mode}&comments_per_page={$comments_per_page}&thread_style={$thread_style}&comments_grandParentId={$comment.parentId}&comments_parentId={$comment.parentId}#form"
							{/if}
