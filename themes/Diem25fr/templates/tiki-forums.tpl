...
{bread_title help="forum" admpage="forums"}{tr}Forums{/tr}{/bread_title}
{/block}


{if $tiki_p_admin_forum eq 'y'}
	{jq}
		$('#TitleBreadActions').append('<li><a href="tiki-admin_forums.php">{tr}Admin{/tr}</a></li>');
	{/jq}
{/if}
...
{if ($tiki_p_forum_read eq 'y' and $prefs.feature_forum_rankings eq 'y')}
	{button href="tiki-forum_rankings.php" _xtype="link" class="btn btn-link" _icon_name="ranking" _text="{tr}Rankings{/tr}"}
{/if}
	</div>
	...
	{if $channels or ($find ne '')}
		...
		{if $prefs.feature_forums_search eq 'y' or $prefs.feature_forums_name_search eq 'y'}
			{if $prefs.feature_forums_name_search eq 'y'}
				<form method="get" class="form-inline" role="form" action="tiki-forums.php">
			...
			{if $prefs.feature_forums_search eq 'y' and $prefs.feature_search eq 'y'}
				<form class="form-inline" method="get" role="form" action="{if $prefs.feature_search_fulltext neq 'y'}tiki-searchindex.php{else}tiki-searchresults.php{/if}">
							<div class="form-group">
								<div class="input-group">
								<span class="input-group-addon">
									{icon name="search"}
...
<div id="{$ts_tableid}-div" class="{if $js === 'y'}table-responsive{/if} ts-wrapperdiv clearfix row" {if $tsOn}style="visibility:hidden;"{/if}> {*the table-responsive class cuts off dropdown menus *}
	<table id="{$ts_tableid}" class="row table table-striped table-hover table-forum normal" data-count="{$cant|escape}">
